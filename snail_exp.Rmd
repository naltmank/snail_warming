...
coral_labels <- c(expression(italic("P. verrucosa")), expression(italic("P. rus")))
coral_limits <- c("Pocillopora", "Porites")
coral_colors <-  c("Pocillopora" = "#0072B2", "Porites" = "#D55E00")


snail_labels <- c("Control", expression(italic("C.~violacea")), expression(italic("D.~cornus")))
...
my_theme <-
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.text.x = element_text(color = "black", size = 30, hjust = .5, vjust = .5, 
                                   face = "plain"),
        axis.title.x = element_text(color = "black", size = 32, hjust = .5, vjust = 0, 
                                    face = "plain"),
        axis.text.y = element_text(color = "black", size = 30, hjust = .5, vjust = .5, 
                                   face = "plain"),
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.title.y = element_text(color = "black", size = 25, hjust = .5, vjust = 0, face = "plain",
                                    margin = margin(r = 15)),
        legend.key.size = unit(2.4, "cm"), # increase legend size
        legend.text = element_text(size = 20), # increase legend text
        legend.title = element_text(size = 25),
        legend.position = 'bottom', # move legend to bottom
        legend.box = 'horizontal',
        legend.background = element_rect(color = NA)# make legend horizontal
  )
...
# Plot
(choice_plot <-
  ggplot(proportions, aes(x = Snail, y = Proportion*15, fill = Prey)) +
  geom_bar(stat = "identity") +
  geom_text(aes(x = 0.55, y = 17, label = "Fisher's Exact Test\nn = 15\nP < 0.0001"), hjust = 0, size = 14) +
  scale_fill_manual(values = c("NC" = "darkgrey",
                               "Pocillopora" = "#0072B2",
                               "Porites" = "#D55E00"),
                    labels = c("No Choice", coral_labels)) +
  labs(x = "", y = "Host Occupied", fill = "Prey") +
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)),
                     breaks = seq(0, 15, 3)) +  
  scale_x_discrete(labels = c("Coralliophila" = expression(italic(C.~violacea)), "Drupella" = expression(italic(D.~cornus)))) +
  my_theme)
...
sum(fed$R2 > 0.9)/nrow(fed) # 0.953 respo curves had an R2 > 0.9

# create ggplot histogram of R2
(fig_s1 <- ggplot(fed, aes(x = R2)) +
...
# ggsave(here::here("output", "respo_r2_hist.png"), fig_s1, height = 10, width = 15)
...
fed_mod <- glmmTMB(abs(Respiration) ~ Temp*Snail + (1|Tank) + (1|Date), data = fed, family = Gamma(link = "log"))
summary(fed_mod)
...
# fed_mod_2 <- glmmTMB(abs(Respiration) ~ Temp*Snail*Coral + (1|Tank), data = fed, family = Gamma(link = "log"))
# plot(simulateResiduals(fed_mod_2)) # similar to above
# summary(fed_mod_2)
# Anova(fed_mod_2) # no coral interaction - all good
```
...
(respo_plot <- ggplot() +
  ...
  scale_x_discrete(labels = c("Coralliophila" = expression(atop(italic("C. violacea"), "n = 53")),
                              "Drupella" = expression(atop(italic("D. cornus"), "n = 54"))
                              )
                   ) +
...
# Version 1
(growth_plot <- ggplot() +
  geom_boxplot(data = porites, aes(x = Snail, y = Net.change, colour = Snail), outlier.shape = NA, show.legend = F) +
  geom_point(data = porites, aes(x = Snail, y = Net.change, colour = Snail), position = position_jitterdodge(0.3), show.legend = F) +
  ...
  facet_wrap(~ Treatment,
   ...
   scale_colour_manual(values = snail_colors,
                       labels = c("Control" = expression(atop("Control", "    n = 15")),
                                  "Coralliophila" = expression(atop(italic("C. violacea"), " n = 15")),
                                  "Drupella" = expression(atop(italic("D. cornus"), "    n = 15"))
                                  )
                       ) +
  scale_x_discrete(labels = c(
    "Control" = expression(atop("Control", "n = 15")),
    "Coralliophila" = expression(atop(italic("C. violacea"), "n = 15")),
    "Drupella" = expression(atop(italic("D. cornus"), "n = 15"))
                              )
                   ) +
...
(poc_survival_plot <-
  ...
  scale_colour_manual(values = snail_colors,
                      labels = c("Control" = expression(atop("Control", "n = 12")),
                                 "Coralliophila" = expression(atop(italic("C. violacea"), "n = 12")),
                                 "Drupella" = expression(atop(italic("D. cornus"), "n = 12"))
                                 )
                      ) +
...
# ggsave(here::here("output", "growth_survival_plot_v6.png"), growth_surv_panel, height = 18, width = 20)
...
(porites_tissue_loss <- ggplot() +
  geom_boxplot(data = porites_tissue, aes(x = Snail, y = Mortality*100, colour = Treatment), outlier.shape = NA) +
  geom_point(data = porites_tissue,
             aes(x = Snail, y = Mortality*100, colour = Treatment), position = position_jitterdodge(0.3)) +
  scale_colour_manual(values = c("Heated" = "#FE6100", "Ambient" = "#648FFF"),
                      labels = c("Heated" = "Heated (30.5°C)", "Ambient" = "Ambient (28.5°C)")) +
  ...
  labs(y = "Tissue Mortality (%)\n", x = "", title = "a.") +
  geom_segment(aes(x = 1.8, xend = 2.2, y = 120, yend = 120)) +
  geom_text(aes(x = 2, y = 120, label = "*"), size = 14) +
  geom_text(data = porites_tissue_letters, aes(x = Snail, y = 110, label = Letters, colour = Treatment),
            position = position_dodge(0.75), size = 10, show.legend = F) +
   scale_y_continuous( breaks = seq(0,100,20)) +
  geom_text(aes(x = 0.5, y = 70, label = "Temp: P < 0.001\nSnail: P < 0.0001\nInteraction: P = 0.84"), hjust = 0, size = 10) + 
  my_theme)

(pocillopora_tissue_loss <- ggplot() +
  geom_boxplot(data = pocillopora_tissue, aes(x = Snail, y = Mortality*100, colour = Treatment), outlier.shape = NA) +
  ...
  labs(y = "Tissue Mortality (%)\n", x = "", title = "b.") +
  geom_text(data = pocillopora_tissue_letters, aes(x = Snail, y = 110, label = Letters, colour = Treatment),
            position = position_dodge(0.75), size = 10, show.legend = F) +
   scale_y_continuous( breaks = seq(0,100,20)) +
  geom_text(aes(x = 3.5, y = 20, label = "Temp: P = 0.23\nSnail: P = 0.006\nInteraction: P = 0.45"), hjust = 1, size = 10) + 
  my_theme)
...
poc_wax_area_full <- merge(poc_area, poc_wax_weights_subset, by = "Coral.ID")

# fit relationship of planar area to surface area to extract relationship
area_mod <- lm(Surface.area ~ Planar.area, poc_wax_area_full)
summary(area_mod) # intercept = -0.94402 , slope = 0.69687
performance::r2(area_mod) # R2 = 0.607 - not great but good
plot(poc_wax_area_full$Planar.area, poc_wax_area_full$Surface.area, xlab = "Planar area", ylab = "Wax dipped surface area")



```

```{r surface area conversion plot, fig.height = 10, fig.width=12}
(poc_biomass_conversion_plot <-
  ggplot(data = poc_wax_area_full) +
  geom_point(aes(x = Planar.area, y = Surface.area)) +
  geom_smooth(aes(x = Planar.area, y = Surface.area), method = lm, se = TRUE) +
  geom_text(aes(x = 63, y = 10, label = "y = 0.69687x - 0.99402\nR2 = 0.607"), hjust = 1, size = 8) +
  labs(x = expression(Planar ~ Area ~ (cm^2)),
       y = expression(Wax ~ Dipped ~ Surface ~ Area ~ (cm^2))) +
   my_theme
 )

# ggsave(here::here("output", "surface_area_conversion_v1.png"), poc_biomass_conversion_plot, height = 10, width = 12)
```
...
poc_surv_full <- poc_tissue_long %>%
  group_by(Coral.ID) %>%
  summarise(Planar.area = sum(Live.tissue))

# merge with proportional mortality data
poc_surv_tissue <- merge(poc_surv_full,
                         pocillopora_tissue[pocillopora_tissue$Coral.ID %in% unique(poc_surv_full$Coral.ID),],
                         by = "Coral.ID")

# calculate surface area based on relationship between planar area and surface area from wax-dipped corals
poc_surv_tissue$Surface.area <- -0.94402 + 0.69687*poc_surv_tissue$Planar.area
...
```{r poc model}
# simplest
wilcox.test(Biomass ~ Treatment, poc_biomass_full) # P = 0.71

# mixed model
poc_bio_mod <- glmmTMB(Biomass ~ Treatment*Snail + (1|Colony.ID) + (1|Tank), family = Gamma("log"), data = poc_biomass_full)
...
Anova(poc_bio_mod) # X2 = 0.75, P = 0.39
```
...
(porites_biomass_plot <-
  ...
  scale_colour_manual(values = c("Heated" = "#FE6100", "Ambient" = "#648FFF"),
                      labels = c("Heated" = "Heated (30.5°C)", "Ambient" = "Ambient (28.5°C)")) +
  labs(y = expression(Tissue ~ biomass ~ (g ~ cm^-2)),
       x = "", title = "a.") +
...
(pocillopora_biomass_plot <-
  ...
  labs(y = expression(Tissue ~ biomass ~ (g ~ cm^-2)),
       x = "", title = "b.") +
  geom_text(aes(x = 0.5, y = 0.025, label = "Wilxocon Rank Sum Test\nW = 73, P = 0.71"), hjust = 0, size = 10) + 
  my_theme)
```
...
(porites_feeding_plot <-
  ...
  scale_colour_manual(values = c("Heated" = "#FE6100", "Ambient" = "#648FFF"),
                      labels = c("Heated" = "Heated (30.5°C)", "Ambient" = "Ambient (28.5°C)")) +
  labs(y = "Feeding Probability", title = "a.", x = "") + 
  geom_segment(aes(x = 1.8, xend = 2.2, y = 0.95, yend = 0.95)) +
  geom_text(aes(x = 2, y = 1, label = "***"), size = 10) +
...
(pocillopora_feeding_plot <-
  ...
  geom_errorbar(data = pocillopora_feeding_emmeans, aes(x = Treatment, ymin = lower.CI, ymax = upper.CI,
                                                    colour = Treatment), width = 0.15, position = position_dodge(0.3)) +
  scale_x_discrete(labels = c("Heated" = "Heated (30.5°C)", "Ambient" = "Ambient (28.5°C)")) +
  scale_colour_manual(values = c("Heated" = "#FE6100", "Ambient" = "#648FFF"),
                      labels = c("Heated" = "Heated (30.5°C)", "Ambient" = "Ambient (28.5°C)")) +
  labs(y = "Feeding Probability", title = "b.", x = "") + 
...
# ggsave(here::here("output", "feeding_panel_v2.png"), feeding_panel, height = 14, width = 12)
```
